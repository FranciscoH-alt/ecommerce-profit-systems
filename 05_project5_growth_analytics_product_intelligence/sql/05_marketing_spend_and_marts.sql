...
-- ─────────────────────────────────────────────────────────────────────────────
-- Executive Marts (P4-compatible layer)
-- ─────────────────────────────────────────────────────────────────────────────
...
spend AS (
  SELECT date_id, SUM(spend) AS spend
...
WHERE d.date BETWEEN (SELECT MIN(date) FROM dim.dim_date)
                 AND (SELECT MAX(date) FROM dim.dim_date);
...
SELECT
  ...
  CASE WHEN st.spend_total > 0
       THEN ds.revenue * (s.spend / st.spend_total) ELSE NULL END AS revenue_proxy,
  CASE WHEN st.spend_total > 0
       THEN ds.contribution_margin * (s.spend / st.spend_total) ELSE NULL END AS contribution_margin_proxy,
  CASE WHEN s.spend > 0 AND st.spend_total > 0
       THEN (ds.revenue * (s.spend / st.spend_total)) / s.spend ELSE NULL END AS mer_proxy
FROM spend s
